...
static void copy_timestamp_fields(Field *from, Field *to)
{
  DBUG_ASSERT(from->type() == MYSQL_TYPE_TIMESTAMP);
  DBUG_ASSERT(to->type() == MYSQL_TYPE_TIMESTAMP);
  ulong sec_part;
  Field_timestamp *f= static_cast<Field_timestamp*>(from);
  Field_timestamp *t= static_cast<Field_timestamp*>(to);
